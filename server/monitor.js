...
async function checkApiStatus(apiUrl) {
    ...
    try {
        ...
        db.run('INSERT INTO log (rota, status, timestamp) VALUES (?, ?, ?)', [apiUrl, response.status, timestamp]);
    } catch (error) {
        console.error(`Falha ao conectar na API ${apiUrl}:`, error.message);
        db.run('INSERT INTO log (rota, status, timestamp) VALUES (?, ?, ?)', [apiUrl, 999, timestamp]);
    }
}
